...
function mb2_shortcode_slider($atts, $content= null){
	...
	$sData = theme_mb2cg_shortcodes_slider_data($carousel_opt);
...
function mb2_shortcode_slider_item($atts, $content = null){
	...
	$color_style = (($GLOBALS['prestyle'] === 'cognitio' || $GLOBALS['prestyle'] === 'cognitio2') && $color) ?
	...
	if ($content || $desc || $title)
	{
		...
		$output .= '<div class="theme-slide-content4"' . $color_style . '>';
		...
		if ($desc || $content)
		{
			...
			if ($link && $link_type == 1)
			{
				$readmoretext = $GLOBALS['readmoretext'] ? $GLOBALS['readmoretext'] : get_string('readmore','theme_mb2cg');
