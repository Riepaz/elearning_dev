...
function mb2_shortcode_courses($atts, $content= null){
	...
	if ($itemCount>1)
	{
		$output .= theme_mb2cg_shortcodes_content_template($courses, $items_opt);
	}
	else
	{
		$output .= get_string('nothingtodisplay');

		if (in_array(theme_mb2cg_site_access(),array('admin','manager','coursecreator')))
		{
			$output .= '<div>';
			$output .= '<a href="' . new moodle_url($CFG->wwwroot . '/course/edit.php',array('category'=>theme_mb2cg_get_category()->id)) . '">' .
...
function mb2_shortcode_courses_get_items ($options)
{
	...
	if ($itemCount>0)
	{
		foreach ($coursesList as $course)
		{
			...
			$imgUrlAtt = theme_mb2cg_shortcodes_content_get_image(array(), false, '', $course->id);
			$imgNameAtt = theme_mb2cg_shortcodes_content_get_image(array(), true, '',  $course->id);
			...
			if ((isset($cat->visible) && !$cat->visible) && $coursecat_canmanage)
			{
				$course->details = $cat->get_formatted_name() . ' (' . get_string('hidden','theme_mb2cg') . ')';
			}
			...
			{
				$course->title .= ' (' . get_string('hidden','theme_mb2cg') . ')';
			}

			$course->redmoretext = get_string('readmore', 'theme_mb2cg');
			$price = mb2_shortcode_courses_course_price($course->id, $options);
			$course->price = '';

			if ($options['courseprices'])
			{
				$course->price = $price ? $price : '<span class="freeprice">' . get_string('noprice','theme_mb2cg') . '</span>';
			}
