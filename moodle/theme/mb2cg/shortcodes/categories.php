...
function mb2_shortcode_categories($atts, $content= null){
	...
	$categories = theme_mb2cg_shortcodes_categories_get_items($items_opt);
	...
	$output .= '<div class="mb2-pb-content mb2-pb-categories' . $cls . '">';
	$output .= '<div class="mb2-pb-content-inner clearfix">';
	$output .= '<div class="mb2-pb-content-list' . $list_cls . '"' . $slider_data . '>';

	if ($itemCount>0)
	{
		$output .= theme_mb2cg_shortcodes_content_template($categories, $items_opt);
	}
	else
	{
 ...
 *
 */
function theme_mb2cg_shortcodes_categories_get_items ($options)
{
	...
	if ($itemCount>0)
	{
		foreach ($categories as $category)
		{
			...
			$imgUrlAtt = theme_mb2cg_shortcodes_content_get_image($image_options, false, $category->description);
			$imgNameAtt = theme_mb2cg_shortcodes_content_get_image($image_options, true, $category->description);
			...
			{
				$category->title .= ' (' . get_string('hidden','theme_mb2cg') . ')';
			}
			...
			$category->details = $courseCount > 0 ? $courseCount . ' ' . $courseString : get_string('nocourseincategory', 'theme_mb2cg');
