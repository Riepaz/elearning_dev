...
class block_mb2slider extends block_base
{
	...
	function mb2slider_check_file_area($cid, $area)
	{

		$fs = get_file_storage();
		$systemfiles = $fs->get_area_files($cid, 'block_mb2slider', $area);
	...
	function mb2slider_item($item, $carousel, $counter, $itemCount,  $course = NULL)
	{
		...
		$sizeTitleArrM = 'px';
		$sizecDescArrM = 'px';
		$lineHeight = round($sizecDesc * 1.65);
		...
		if (isset($sizecDescArr[1]))
		{
			...
			$sizecDescArrM = 'rem';
			$lineHeight = round($sizecDesc * 1.65, 3);
		}
		...
		if ($imageUrl)
		{
			...
			if ($title || $showDesc)
			{
				...
				if ($showDesc)
				{
					...
					$output .= $showText ? '<div class="mb2slider-text" style="font-size:' . $sizecDesc . $sizecDescArrM  . ';line-height:' . $lineHeight . $sizecDescArrM . ';">' . $isText . '</div>' : '';
	...
	function mb2slider_image_url($imagenum, $name = false, $course = NULL)
	{
		...
		$i = -1;
		$fs = get_file_storage();
		$files = $fs->get_area_files($this->context->id, 'block_mb2slider', 'content');
		$systemfiles = $fs->get_area_files($systemcontext->id, 'block_mb2slider', 'configimages');
		$configimages = $this->mb2slider_check_file_area(context_system::instance()->id, 'configimages');
